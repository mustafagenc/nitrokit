// src/lib/auth/two-factor-service.ts
...
export class TwoFactorService {
    ...
    private static async computeHMAC(key: Uint8Array, data: Uint8Array): Promise<Uint8Array> {
        ...
        const cryptoKey = await crypto.subtle.importKey(
            'raw',
            keyBuffer, // ✅ ArrayBuffer kullan
            { name: 'HMAC', hash: 'SHA-1' },
            false,
            ['sign']
        );

        const signature = await crypto.subtle.sign('HMAC', cryptoKey, dataBuffer); // ✅ ArrayBuffer kullan
    ...
    static async generateSetup(userId: string, userEmail: string): Promise<TwoFactorSetup> {
        try {
            // Generate 20-byte secret using Web Crypto API
    ...
    static async disable(userId: string, token: string): Promise<boolean> {
        try {
            ...
            const verified = await this.verifyTOTP(user.twoFactorSecret, token);
    ...
    static async verifyToken(userId: string, token: string): Promise<TwoFactorVerification> {
        try {
            ...
            const totpVerified = await this.verifyTOTP(user.twoFactorSecret, token);
